"""
Dashboard Credit Scoring Production - Corrigé
"""
...
def get_bivariate_data_cached(var1, var2):
   """Analyse bi-variée avec cache 30min"""
   ...
   if result:
       ...
       st.error(f"Erreur analyse bi-variée: {error}")
       return None
...
def display_simple_population_comparison(client_data):
   """Interface comparaison population avec cache optimisé"""
   ...
   # Clé de cache consistante
   cache_key = CacheManager.get_cache_key("population_data", selected_variable)
   
   # Vérifier cache en session state
   if cache_key in st.session_state:
       col2a, col2b = st.columns(2)
       with col2a:
           st.success("✅ En cache")
       with col2b:
           if st.button("🔄", help="Actualiser", key="refresh_population"):
       ...
       with col2:
           if st.button("📊 Charger", help="Charger les données", key="load_population_btn"):
               with st.spinner("🔄 Chargement..."):
                   distribution_data = get_population_distribution_cached(selected_variable)
               
               if distribution_data:
                   client_value = client_data.get(selected_variable)
                   if client_value is not None:
                       # Stocker dans session state
                       st.session_state[cache_key] = distribution_data
                       st.success(f"✅ {FEATURE_TRANSLATIONS.get(selected_variable, selected_variable)} chargé")
                       st.rerun()
                   else:
                       st.error(f"Valeur client manquante pour {selected_variable}")

def create_bivariate_plot(x_data, y_data, var1, var2, client_data):
   """Créer graphique bi-varié centralisé"""
   
   # Conversion avec fonctions centralisées
   x_data_converted = convert_categorical_values(x_data, var1)
   y_data_converted = convert_categorical_values(y_data, var2)
   
   # Assurer même longueur
   min_len = min(len(x_data_converted), len(y_data_converted))
   x_final = x_data_converted[:min_len]
   y_final = y_data_converted[:min_len]
   ...
   # Position client
   client_x = convert_client_value(client_data.get(var1, 0), var1)
   client_y = convert_client_value(client_data.get(var2, 0), var2)
   ...
   correlation = np.corrcoef(x_final, y_final)[0, 1] if len(x_final) > 1 else 0
...
if not st.session_state.client_analyzed:
   ...
   # Bouton avec Appel API optimisé
   ...
   with col2:
       if st.button(
           ...
           with st.spinner("🔄 Analyse en cours..."):
               client_data_json = json.dumps(client_data, sort_keys=True)
               result, error = call_prediction_api_cached(client_data_json)
   ...
   with tab2:
       ...
       # Interface comparaison population optimisée
   ...
   with tab3:
       ...
       # Cache key pour analyse bi-variée
       bivariate_cache_key = CacheManager.get_cache_key("bivariate", var1, var2)
       
       # Gestion cache bi-variée
       if bivariate_cache_key in st.session_state:
           col1, col2 = st.columns([3, 1])
           with col1:
               st.success("✅ Analyse en cache")
           with col2:
               if st.button("🔄", help="Actualiser analyse", key="refresh_bivariate"):
                   del st.session_state[bivariate_cache_key]
                   st.rerun()
           
           # Afficher depuis cache
           cached_data = st.session_state[bivariate_cache_key]
           ...
           if st.button("📈 Analyser la relation", use_container_width=True, key="analyze_bivariate_btn"):
               ...
               with st.spinner("🔄 Analyse bi-variée en cours..."):
                   ...
                   dist1 = get_population_distribution_cached(var1)
                   dist2 = get_population_distribution_cached(var2)
               ...
               if dist1 and dist2:
                   ...
                   if values1 and values2:
                       # Stocker en cache
                       st.session_state[bivariate_cache_key] = {
                           'x_data': values1,
                           'y_data': values2,
                           'var1': var1,
                           'var2': var2
                       ...
                       # Afficher graphique
                       create_bivariate_plot(values1, values2, var1, var2, st.session_state.client_data)
                   else:
